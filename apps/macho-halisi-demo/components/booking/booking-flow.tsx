...
const addons = [
  { id: "vehicle", label: "Private Game Drive Vehicle", price: 220, per: "per day" },
  { id: "balloon", label: "Hot Air Balloon Safari over the Serengeti", price: 600, per: "per person" },
  { id: "beachext", label: "Extra Night Beach Extension (Zanzibar)", price: 180, per: "per person, per night" },
  { id: "privateguide", label: "Private Guide Upgrade", price: 150, per: "per day" },
  { id: "airport", label: "Airport Transfer (Kilimanjaro Intl. Airport – JRO)", price: 60, per: "one-way" },
]
...
export function BookingFlow() {
  ...
  const addonsTotal = useMemo(() => {
    return selectedAddons.reduce((sum, id) => {
      ...
      if (!a) return sum
      if (id === "beachext") return sum + a.price * adults * nights // per person, per night
      if (id === "vehicle" || id === "privateguide") return sum + a.price * nights // per day
      if (id === "balloon") return sum + a.price * adults // per person, one-time
      return sum + a.price // flat, one-time (e.g. airport transfer)
    }, 0)
  ...
  const handleConfirm = () => {
    toast.success("Enquiry received — a confirmation has been sent to your email.")
    setStep(6)
  }
...
function StepDates({
  ...
  return (
    <div>
      <h2 className="font-serif text-3xl mb-2">When does your trip start?</h2>
      <p className="text-sm text-muted-foreground mb-8">
        Choose your dates and the number of travelers joining you.
      </p>
...
function StepRoom({
  ...
  return (
    <div>
      <h2 className="font-serif text-3xl mb-2">Choose your tour</h2>
      <p className="text-sm text-muted-foreground mb-8">
        {nights} day{nights !== 1 ? "s" : ""} · Rates shown are per person, per day
      </p>
...
function StepAddons({
  ...
  return (
    <div>
      <h2 className="font-serif text-3xl mb-2">Enhance your trip</h2>
      <p className="text-sm text-muted-foreground mb-8">
        Optional additions to curate your itinerary. All can be added later.
      </p>
...
function StepGuest({
  ...
  return (
    <div>
      <h2 className="font-serif text-3xl mb-2">Traveler details</h2>
      <p className="text-sm text-muted-foreground mb-8">
        Primarily for your enquiry — we&apos;ll follow up by name.
      </p>
      <div className="grid md:grid-cols-2 gap-5">
        <div>
        ...
        <div className="md:col-span-2">
          ...
          <Textarea id="s" value={guest.special} onChange={(e) => update("special", e.target.value)} rows={4} className="rounded-none resize-none" placeholder="Dietary needs, fitness level, occasions..." />
        </div>
      </div>
    </div>
  )
...
function StepPayment({ total, onConfirm }: { total: number; onConfirm: () => void }) {
  ...
  return (
    <div>
      ...
      >
        {loading ? "Processing..." : `Submit Enquiry · $${total}`}
      </Button>

      <p className="mt-4 text-xs text-center text-muted-foreground">
        Cancellation and payment terms will be confirmed directly with our team.
      </p>
    </div>
  )
}
  ...
  return (
    <div className="text-center py-8">
      ...
      <p className="mt-4 text-muted-foreground max-w-md mx-auto leading-relaxed">
        Your safari enquiry is confirmed. A confirmation has been sent to{" "}
        <span className="text-foreground">{guest.email || "your email"}</span>.
        Our team will be in touch to help plan your trip.
      </p>
      <div className="mt-8 text-[11px] uppercase tracking-[0.24em] text-muted-foreground">
        Booking Reference · MH-{Math.random().toString(36).slice(2, 8).toUpperCase()}
      </div>
    </div>
  )
}
...
function Summary({
  ...
  return (
    <div className="bg-navy text-background p-6 md:p-8">
      <p className="eyebrow text-accent mb-5">Your trip summary</p>
      ...
      <div className="mt-6 space-y-3 text-sm border-t border-white/10 pt-5">
        <Row label="Start date" value={format(checkIn, "EEE, MMM d")} />
        <Row label="End date" value={format(checkOut, "EEE, MMM d")} />
        <Row label="Travelers" value={`${adults + children} · ${nights} day${nights !== 1 ? "s" : ""}`} />
      </div>

      <div className="mt-5 pt-5 border-t border-white/10 space-y-3 text-sm">
        <Row label={`Tour × ${nights}`} value={`$${roomTotal}`} />
